    ...
    <div className="max-w-[1400px] mx-auto px-4 py-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-4">
        <div className="bg-white rounded-lg divide-y divide-gray-100">
          <div className="px-4 py-2 text-xs text-gray-500 grid grid-cols-[80px_1fr_140px_120px_40px] gap-3 items-center">
            <div></div><div>Product</div><div className="text-center">Price</div><div className="text-center">Quantity</div><div></div>
          </div>
          {cart.map(item => (
            <div key={item.id} className="px-4 py-4 grid grid-cols-[80px_1fr_140px_120px_40px] gap-3 items-center">
        ...
        <aside className="bg-white rounded-lg p-5 h-fit sticky top-32">
          ...
          <button onClick={() => user ? navigate('/checkout') : navigate('/login')} className="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold py-3 rounded mt-4 transition-colors">
            Proceed to Checkout
          </button>
